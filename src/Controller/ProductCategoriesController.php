...
class ProductCategoriesController extends AppController
{
    ...
    {
        $this->paginate = [
            'contain' => ['Images', 'ProductRays'],
            'order' => ['ProductCategories.id' => 'ASC']
        ];
    ...
    public function view($id = null)
    {
        $productCategory = $this->ProductCategories->get($id, [
            'contain' => ['Images', 'ProductRays', 'Products'],
        ]);
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $productCategory->image = $this->ProductCategories->Images->patchEntity(
                $this->ProductCategories->Images->newEntity(),
                [
                    'folder' => 'product_categories',
                    'url' => 'default.jpg',
                    'link' => self::IMG_PROD_URL . 'product_categories/default.jpg'
                ]
            );
        ...
        $images = $this->ProductCategories->Images->find('list', ['limit' => 200]);
        $productRays = $this->ProductCategories->ProductRays->find('list', ['limit' => 200]);
        $this->set(compact('productCategory', 'images', 'productRays'));
    }
